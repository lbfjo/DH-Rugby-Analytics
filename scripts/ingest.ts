...
function parseTimeToSeconds(value: unknown): number {
  if (value === null || value === undefined) return 0;
    ...
    if (value instanceof Date) {
      // For time-only values, Excel stores as fraction of day
      // The Date will have hours/minutes/seconds
              ...
              if (year === 1899 && month === 11) {
                ...
                daysOffset = day + 1; // 31 days in Dec + current day - 30 = 1 + day? No. 
                // Dec 30 = 0. Dec 31 = 1. Jan 1 = 2.
                // So Jan 1: 31 - 30 + 1 = 2. Correct.
                daysOffset = (31 - 30) + day;
              } else {
                  // Fallback for unlikely dates far from epoch (just use getHours)
                  daysOffset = 0;
              }
              ...
              if (totalHours > 2) {
                 // Treat Total Hours as Minutes
                 // Total Minutes = totalHours * 60 + minutes? No.
                 // The "Hours" part is the minutes. The "Minutes" part is the seconds.
                 // The "Seconds" part is 0 (usually).
  ...
  if (typeof value === "number") {
    ...
    // 0.5 = 12:00:00 (noon)
    // For "28:06" stored as time, it would be 28/1440 days ≈ 0.0195
    // But if stored as duration > 24h, it's different
    // We'll treat small numbers as fraction of day
    if (value < 1) {
    ...
    // If > 1, it's likely total seconds or minutes already
...
function parsePossessionToSeconds(value: unknown): number {
  ...
  if (value instanceof Date) {
    // Hours field contains minutes, minutes field contains seconds
